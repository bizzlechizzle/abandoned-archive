...
```bash
# Parse a KML file
mapcombine parse saved-places.kml -o points.json

# Deduplicate multiple map files
mapcombine dedup *.kml *.gpx -o deduped.geojson -f geojson

# Compare two location names
mapcombine compare "PRR Station" "Pennsylvania Railroad Depot"
```
...
```bash
mapcombine parse <files...> [options]
...
```bash
# Parse single file
mapcombine parse places.kml

# Parse multiple files, output as GeoJSON
mapcombine parse *.kml *.gpx -f geojson -o combined.geojson

# Parse and view as table
mapcombine parse places.gpx -f table
```
...
```bash
mapcombine dedup <files...> [options]
...
```bash
# Basic deduplication
mapcombine dedup maps/*.kml -o deduped.json

# Strict GPS matching (25m)
mapcombine dedup *.kml -g 25 -o strict.geojson

# Require GPS match, verbose output
mapcombine dedup *.kml --require-gps -v

# Output as GeoJSON for mapping
mapcombine dedup *.kml -f geojson -o locations.geojson
```
...
```bash
mapcombine compare <name1> <name2> [options]
...
mapcombine compare "Union Station - Lockport" "Lockport Union Train Station"

# Check blocking conflict
mapcombine compare "North Factory" "South Factory"
```
...
```bash
mapcombine match <reference> <target> [options]
...
mapcombine match database.geojson new-finds.gpx -o matches.json
```
...
```bash
mapcombine merge <files...> [options]
...
```bash
mapcombine merge *.kml *.gpx -o all-points.geojson
```
...
```bash
mapcombine stats <files...> [options]
