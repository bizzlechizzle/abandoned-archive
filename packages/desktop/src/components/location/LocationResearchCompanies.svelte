<script lang="ts">
  /**
   * LocationResearchCompanies - Organizations entities nested accordion
   * Extracted companies/orgs from web sources, nested within Research section
...
<div class="border-b border-braun-200 last:border-b-0">
  ...
  >
    ...
    <div class="flex items-center gap-2">
      {#if !loading}
        <span class="text-sm text-braun-400">({organizations.length})</span>
      {/if}
      <svg
