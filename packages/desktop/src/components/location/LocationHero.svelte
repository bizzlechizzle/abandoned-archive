...
{#if images.length > 0 && heroImage}
  <div class="w-full bg-[#FAFAF8]">
    ...
    >
      {#if heroSrc}
        ...
        {#if isLowQuality && onRegeneratePreview}
          <div class="absolute top-3 right-3 z-10 flex items-center gap-2">
            <span class="px-2 py-1 bg-warning text-white text-xs font-medium rounded flex items-center gap-1">
            ...
            <button
              ...
              class="px-2 py-1 bg-white/90 hover:bg-white text-braun-900 text-xs font-medium rounded flex items-center gap-1 transition disabled:opacity-50"
              title="Regenerate higher quality preview"
            >
              {#if regenerating}
                <div class="w-3 h-3 border border-braun-600 rounded flex items-center justify-center">
                  <div class="w-1.5 h-1.5 bg-braun-600 rounded"></div>
                </div>
              {:else}
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {/if}
        {#if regenerateError}
          <div class="absolute top-12 right-3 z-10 px-2 py-1 bg-error text-white text-xs rounded">
            {regenerateError}
          </div>
        {/if}
      {:else}
      ...
      {/if}
      <!-- Braun: Pure photography - no gradient overlay. Clean bottom edge only. -->
    </div>
  </div>
{:else}
  <!-- PUEA: Graceful empty state - matches hero constraints -->
  <div class="w-full bg-[#FAFAF8]">
    <div class="w-full max-h-[40vh] mx-auto bg-braun-100 flex items-center justify-center" style="aspect-ratio: 2.35 / 1;">
