<script lang="ts">
  ...
  // OPT-109: Web-extracted media filter
  ...
  // OPT-109: Calculate web-extracted counts
  ...
  // Filtered media based on showHidden and showWebExtracted state
  ...
  const visibleVideos = $derived(
    videos.filter(v => {
      if (v.hidden === 1 && !showHidden) return false;
      if (v.extracted_from_web === 1 && !showWebExtracted) return false;
      return true;
    })
  );
...
<div class="mt-6 bg-white rounded border border-braun-300">
  ...
  {#if isOpen}
    <div class="px-6 pb-6 space-y-2">
      <!-- Filter toggles -->
      {#if totalHiddenCount > 0 || totalWebCount > 0}
        <div class="flex items-center justify-end gap-2 mb-2 flex-wrap">
          <!-- OPT-109: Web-extracted filter -->
          {#if totalWebCount > 0}
            <button
              onclick={() => showWebExtracted = !showWebExtracted}
              class="text-sm flex items-center gap-2 px-3 py-1.5 rounded transition-colors {showWebExtracted ? 'bg-blue-100 text-blue-700' : 'bg-braun-100 text-braun-600 hover:bg-braun-200'}"
              title="Toggle web-extracted media visibility"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" />
              </svg>
              {showWebExtracted ? `Web (${totalWebCount})` : `Show Web (${totalWebCount})`}
            </button>
          {/if}
